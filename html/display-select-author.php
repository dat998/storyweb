       ...
       <?php
        ...
        <main class="truyen-main" style="background:none;>
            <div class="container" style="width: 80%;height: 100%; margin-left: 10%; margin-right: 10%;">
                <form method="GET" action="./display-add-story.php">
                    <h4>Chọn tác giả để thêm truyện mới <br></h4>
                        <?php
                            $id = $_SESSION['id'];
                            $sql = "SELECT author_name From author WHERE userName='$id'";
                            $result = $conn->query($sql);
                            //echo $_SESSION['id'];
                            while($row = $result->fetch_assoc()){
                                $nameAuthor=$row["author_name"];
                                $_SESSION['author'] = $row['author_name'];
                                echo'<a href="./display-add-story.php?author='.$row['author_name'].'">';
                                echo $row['author_name'];
                            // echo'<button type="submit" value= "'.$nameAuthor.'" name="author">'.$nameAuthor.'</button>';
                                echo' </a><hr>';
                            }
                        ?>
                </form>
